...
import { Layout, Menu, Input, Tree, Button, Typography, ConfigProvider, theme, Dropdown, Spin } from 'antd';
import { Outlet, useNavigate, useLocation } from 'react-router-dom';
import { Sun, Moon, Search, Menu as MenuIcon, Settings, User, LogOut, Home, Bell, SlidersHorizontal } from 'lucide-react';
import { fetchEcowatchAreaTree } from './ecowatchApi';
...
export default function MainLayout() {
  ...
  useEffect(() => {
    ...
    const loadAreas = async () => {
      ...
      try {
        const mappedTree = await fetchEcowatchAreaTree();
        if (cancelled) return;
        setTreeData(mappedTree);
      } catch (error) {
        if (!cancelled) {
          console.error('Failed to load project devices for ecowatch area tree:', error);
          setTreeData([]);
        }
      } finally {
        if (!cancelled) {
            ...
            >
              ...
              <Spin spinning={loadingAreas}>
                <Tree
                  checkable
                  showLine
                  defaultExpandedKeys={['default:MAIN_ELECTRICAL', 'default:RAC']}
                  treeData={treeData}
                  ...
                  onSelect={(keys, info) => {
                    ...
                    if (newCheckedKeys.includes(clickedKey)) {
                      newCheckedKeys = newCheckedKeys.filter((key) => key !== clickedKey);
                      newCheckedNames = newCheckedNames.filter((name) => name !== clickedTitle);
                    } else {
