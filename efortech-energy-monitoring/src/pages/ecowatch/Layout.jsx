...
const menuItems = [
  {
    ...
    children: [
      { key: '/ecowatch/area-usage', label: 'Area Usage' },
      { key: '/ecowatch/demand', label: 'Demand' },
      { key: '/ecowatch/tou-period', label: 'TOU Period' },
      { key: '/ecowatch/item-summary', label: 'Item Summary' },
    ],
  },
  {
    key: 'effect-analysis',
    ...
    children: [
      { key: '/ecowatch/energy-ranking', label: 'Energy Ranking' },
      { key: '/ecowatch/energy-flow', label: 'Energy Flow' },
      { key: '/ecowatch/loss-analysis', label: 'Loss Analysis' },
    ],
  },
  {
    key: 'report-management',
    label: 'Report Management',
    children: [{ key: '/ecowatch/annual-report', label: 'Annual Report' }],
  },
];
    ...
    children: [
      {
        title: 'RAC',
        key: '0-0-0',
        children: [{ title: 'LVMDP_RAC', key: '0-0-0-0' }],
      },
      {
        title: 'NR1',
        key: '0-0-1',
...
async function fetchProjectEnergyTreeNode() {
  try {
    ...
    const devices = (payload.devices || []).filter((device) => {
      if (!device?.deployed) {
        return false;
      }
    ...
    return {
      title: REAL_ROOT,
      key: 'real-root',
      children: devices.map((device, index) => ({
        title: String(device.name || '').trim(),
        key: `real-${index}-${String(device.name || '').trim()}`,
      })),
    };
  } catch (error) {
    console.error('Failed to load project devices for real tree:', error);
    return null;
  }
}
...
export default function MainLayout() {
  ...
  useEffect(() => {
    ...
    const loadTreeNodes = async () => {
      const realNode = await fetchProjectEnergyTreeNode();
      if (cancelled) {
        return;
      }

      setTreeNodes(realNode ? [...treeData, realNode] : treeData);
    };
          ...
          {showAreaSidebar && (
            ...
            >
              ...
              <Tree
                ...
                onCheck={(keys, info) => {
                  setCheckedKeys(keys);
                  setCheckedAreaNames(info.checkedNodes.map((node) => String(node.title)));
                }}
                onSelect={(keys, info) => {
                  setSelectedKeys(keys);
                  const clickedKey = info.node.key;
                  const clickedTitle = String(info.node.title);

                  let nextCheckedKeys = [...checkedKeys];
                  let nextCheckedNames = [...checkedAreaNames];

                  if (nextCheckedKeys.includes(clickedKey)) {
                    nextCheckedKeys = nextCheckedKeys.filter((key) => key !== clickedKey);
                    nextCheckedNames = nextCheckedNames.filter((name) => name !== clickedTitle);
                  } else {
                    nextCheckedKeys.push(clickedKey);
                    nextCheckedNames.push(clickedTitle);
                  }

                  setCheckedKeys(nextCheckedKeys);
                  setCheckedAreaNames(nextCheckedNames);
                }}
              />
            </Sider>
          )}
