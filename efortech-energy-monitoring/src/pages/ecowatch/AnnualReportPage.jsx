...
export default function AnnualReport() {
  ...
  const fetchAnnualData = () => {
    ...
    let url = `/energy?interval=Month&start=${selectedYear}-01-01&end=${selectedYear}-12-31`;
    ...
    axios
      ...
      .then((res) => {
        ...
        const monthlyAggregates = Array.from({ length: 12 }, (_, index) => ({
          key: String(index + 1),
        ...
        const formatted = monthlyAggregates.map((row) => {
  ...
  const displayData = tableData.length > 0 ? [
    ...tableData,
    {
      key: 'total',
  ...
  const handleDownloadPDF = () => {
    ...
    autoTable(doc, {
      head: [[
        ...
        'Comprehensive usage (tce)',
        'Total output value (K IDR)',
        'Total output (pcs)',
        'Total area (m2)',
      ]],
      body: rawExportData.map((row) => [
        row.month,
        row.actualStr,
        row.equivalentStr,
        row.compUsageStr,
