...
export default function AreaUsagePage() {
  ...
  const fetchData = () => {
    setLoading(true);
    let url = `/energy?interval=${intervalWaktu}`;
  ...
  const areaUsageOption = useMemo(() => {
    ...
    const xAxisData = [...new Set(chartData.map((item) => item.timestamp))].sort();
    const tags = [...new Set(chartData.map((item) => item.tag_name))];
    const dataMap = {};

    chartData.forEach((item) => {
      if (!dataMap[item.tag_name]) {
        dataMap[item.tag_name] = {};
      }
      dataMap[item.tag_name][item.timestamp] = item.value_kwh;
    });
