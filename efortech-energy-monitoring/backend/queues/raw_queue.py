...
from core.config import (
    ...
    PROJECT_RAW_QUEUE_DLQ_NAME,
    PROJECT_RAW_QUEUE_GROUP,
    PROJECT_RAW_QUEUE_NAME,
)
...
_queue_client = None
_METRICS_KEY = "project_raw_metrics"


def raw_queue_available() -> bool:
    return PROJECT_QUEUE_ENABLED and redis is not None


def _get_client():
    global _queue_client
    if _queue_client is None and raw_queue_available():
...
def ensure_raw_queue_group():
    client = _get_client()
    if client is None:
        return
    try:
        client.xgroup_create(PROJECT_RAW_QUEUE_NAME, PROJECT_RAW_QUEUE_GROUP, id="0", mkstream=True)
...
def enqueue_raw_job(
    ...
    client = _get_client()
    if client is None:
        raise RuntimeError("Raw queue is not available.")

    try:
        current_length = int(client.xlen(PROJECT_RAW_QUEUE_NAME))
    except Exception:
        current_length = 0
    ...
    if PROJECT_QUEUE_BACKPRESSURE_LIMIT > 0 and current_length >= PROJECT_QUEUE_BACKPRESSURE_LIMIT:
        ...
        raise RuntimeError("Raw queue backpressure limit reached.")
...
def build_raw_consumer_name(worker_name: str, slot: int) -> str:
    hostname = socket.gethostname()
    return f"{worker_name}-{hostname}-{slot}"


def read_raw_jobs(consumer_name: str) -> list[tuple[str, dict[str, str]]]:
    client = _get_client()
    if client is None:
        raise RuntimeError("Raw queue is not available.")
    ...
    response = client.xreadgroup(
        groupname=PROJECT_RAW_QUEUE_GROUP,
        consumername=consumer_name,
        streams={PROJECT_RAW_QUEUE_NAME: ">"},
...
def ack_raw_job(message_id: str):
    client = _get_client()
    if client is None:
        raise RuntimeError("Raw queue is not available.")
    client.xack(PROJECT_RAW_QUEUE_NAME, PROJECT_RAW_QUEUE_GROUP, message_id)
    try:
        client.xdel(PROJECT_RAW_QUEUE_NAME, message_id)
    except Exception:
        pass


def retry_or_dead_letter_raw_job(message_id: str, payload: dict[str, str], error_message: str):
    client = _get_client()
    if client is None:
        raise RuntimeError("Raw queue is not available.")
...
def claim_stale_raw_jobs(consumer_name: str) -> list[tuple[str, dict[str, str]]]:
    client = _get_client()
    if client is None:
        raise RuntimeError("Raw queue is not available.")
...
def raw_queue_metrics() -> dict[str, int]:
    client = _get_client()
    if client is None:
        return {
            "queueDepth": 0,
    ...
    try:
        metrics["queueDepth"] = int(client.xlen(PROJECT_RAW_QUEUE_NAME))
    except Exception:
        pass
    try:
        metrics["dlqDepth"] = int(client.xlen(PROJECT_RAW_QUEUE_DLQ_NAME))
    except Exception:
        pass
    try:
        pending_summary = client.xpending(PROJECT_RAW_QUEUE_NAME, PROJECT_RAW_QUEUE_GROUP)
